...
[Pulse](https://github.com/ciphera-net/pulse) is open source, and so is [Tessera](https://github.com/ciphera-net/tessera) — the OPAQUE authentication core and SDKs behind our zero-knowledge claims, Apache-2.0, live on crates.io and npm since June 2026 ([tessera-go](https://github.com/ciphera-net/tessera-go), [@ciphera-net/tessera](https://www.npmjs.com/package/@ciphera-net/tessera)). That's the cryptography itself, independently auditable — a stronger receipt than a UI kit. The Linux Foundation (2025) found 78% of enterprises cite improved security from open source because code is inspectable by anyone, not just the company that wrote it.

### What does zero-knowledge encryption mean in practice?

It means Ciphera's servers can't read your files. Data is encrypted with AES-256-GCM in your browser before upload. Authentication itself is OPAQUE (RFC 9807) — no password-derived value ever reaches the server. The vault key is a non-extractable browser CryptoKey, stored wrapped under the OPAQUE export_key that only a successful login can re-derive; SRP-6a, the protocol this replaced, was retired in June 2026. Even in a worst-case server breach, attackers find only encrypted blobs and wrapped keys they can't unwrap. See our [zero-knowledge encryption guide](https://ciphera.net/blog/zero-knowledge-encryption-guide) for the full flow.

### Does Ciphera sell any user data?

No. We don't collect personal data, so there's nothing to sell. Pulse collects zero personal data by architecture. The data broker market is worth $315.66 billion in 2026, up from $294.27 billion in 2025 ([Mordor Intelligence](https://www.mordorintelligence.com/industry-reports/data-broker-market)). We opted out of that economy entirely.
