...
faqs:
  ...
  - question: "What happens if I forget my Ciphera password?"
    answer: "Account recovery is unavailable at the moment: we are completing a change to the recovery ceremony, and until it ships your password is the only way into your account. Lose it today and nobody can restore access, including us. The 24-word recovery phrase your device generated at signup is still worth keeping — its wrapped key is stored, so the phrase becomes the way back in once recovery is switched on. That is not a support limitation; it is the security property working as designed, and the honest version of it."
...
*Update, 03-09-2026 — account recovery is currently switched off, and earlier versions of this post said otherwise. The server ceremony that proves you hold the recovery phrase shipped in August; the browser half of it has not, so no phrase can currently open an account and no account has enrolled one. Until that ships, **your password is the only way into your account** — losing it alone is terminal, not "losing both". The 24-word phrase is still worth keeping: its wrapped vault key is already stored, so the phrase becomes the way back in the moment recovery is re-enabled. Every claim below has been corrected to say that.*
...
**User error.** Losing your password means losing your data — there is no backdoor for us to helpfully use, and with recovery switched off there is no second key either. That trade-off is the entire point, and you should walk into it with open eyes. When recovery is back on, the 24-word phrase becomes the second key and the sentence becomes "losing both".
...
2. **Does account recovery work without a recovery secret *you* generated?** If yes, that's a backdoor with a friendlier name. (Ours will require the 24-word phrase your device minted at signup. It is switched off today, which is the fail-closed direction: no recovery at all rather than one we could drive ourselves.)
